#ifndef CLASSIC_FLANGER_PRESET_H
#define CLASSIC_FLANGER_PRESET_H
...
{
    std::string name;
    float range     = paramInfo[pParamRange].defaultVal;
    float fine      = paramInfo[pParamFine].defaultVal;
    float rate      = paramInfo[pParamRate].defaultVal;
    float depth     = paramInfo[pParamDepth].defaultVal;
    float mix       = paramInfo[pParamMix].defaultVal;
...
{
public:
    explicit PresetManager(ClassicChorusUI* ui);
    ...
    // (called from ClassicFlangerUI::stateChanged)
    void restoreFromState(const std::string& typeStr,
                          const std::string& nameStr,
                          bool modified);

private:
    ClassicChorusUI*   fUI           = nullptr;
...
#endif // CLASSIC_FLANGER_PRESET_H
